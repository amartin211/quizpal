...
def put_image_to_s3(image_path,filename):
   bucket_processed_img = "bucketforprocessedimg"
   image_data = read_image_as_binary_file(image_path)
   s3.put_object(Bucket=bucket_processed_img, Key=filename, Body=image_data)


def get_connection_id(connectionID_table):
    # This is an example. You need to adjust this function to select the correct connection ID.
    response = connectionID_table.scan()  # You might want to use query() for efficiency if your table is large
    items = response['Items']
    
    if not items:
        raise Exception("No connection ID found in DynamoDB")
    
    # Here, simply taking the first connection ID found. Adjust as necessary.
